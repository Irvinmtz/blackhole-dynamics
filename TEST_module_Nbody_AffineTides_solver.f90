...
integer												:: use_12PN, use_25PN, Identify_3Body_endstate, max_sim_nrsteps
...
CONTAINS
	...
	!----------------------------------------------------------------------------------------------------	
		...
		!initilize:
		r_ij(:,:) = 1e8	!=high number to make use MIN(r/..) is not i,i.
		v_ij(:,:) = 1d0	!=1 to avoid divergence at i,i when calc dt.
		a_ij(:,:) = 1d0 !=1 to avoid divergence at i,i when calc dt.
		!i,j loop:
	...
	!----------------------------------------------------------------------------------------------------
	SUBROUTINE  Nbody_coll_tidaldisrup_sub(Y, Return_Nbody_coll_tidaldisrup)
	!----------------------------------------------------------------------------------------------------
		IMPLICIT NONE
		real*8, dimension(tot_nr_Y_evol_eqs),	intent(in)		:: Y
	...
	!----------------------------------------------------------------------------------------------------
		...
		real*8, dimension(3)								:: acc_CM_pointmass_NT_ij, acc_CM_pointmass_25PN_ij
		...
		!------------------------------------------------------------	
			...
			if (i .NE. j) then			
				...
				acc_tot_CM_ij		= acc_CM_pointmass_NT_ij + acc_CM_pointmass_25PN_ij + acc_CM_tidalfield_ij
				!save and sum terms:
	...
	!solver params:
	read(*,*), Nbody_solver_params_1_INT(:)		! [use_12PN, use_25PN, Identify_3Body_endstate, max_sim_nrsteps, FREE, outputinfo_screenfiles, nr_nbody_particles, ...]
	...
	!Define:
	!--------------------------------------------------------
	!Nbody_solver_params_1_INT:  [use_12PN, use_25PN, Identify_3Body_endstate, max_sim_nrsteps, FREE, outputinfo_screenfiles, nr_nbody_particles, ...]
	...
	Identify_3Body_endstate 		= Nbody_solver_params_1_INT(3)
	...
	DO WHILE (end_simulation .EQ. 0)			
		...
		!Check for collisions:
		!-----------------------------------
		CALL Nbody_coll_tidaldisrup_sub(Y, Return_Nbody_coll_tidaldisrup)
	...
	if (outputinfo_screenfiles .EQ. 1) then
		...
		print*, endsim_Return_Info_arr_REAL
